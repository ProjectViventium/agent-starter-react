...
// Image metadata
// VIVENTIUM START
// Purpose: Ensure social share metadata reflects Viventium branding.
// Details: docs/requirements_and_learnings/16_Branding_and_Assets.md#agent-starter-opengraph
// VIVENTIUM END
export const alt = 'Viventium Voice Assistant';
export const size = {
  width: 1200,
  height: 628,
};
...
export default async function Image() {
  ...
  const rawLogoUri = appConfig.logoDark || appConfig.logo;
  const isLogoUriRemote = isRemoteFile(rawLogoUri);
  const logoUri = isLogoUriRemote ? rawLogoUri : `public/${rawLogoUri.replace(/^\//, '')}`;
  ...
  const wordmarkSize = scaleImageSize(wordmarkDimensions, isLogoUriRemote ? 64 : 32);

  // logo
  const { base64: logoSrcBase64, dimensions: logoDimensions } = await getImageData(
    logoUri,
    'public/viventium-logo.svg'
  );
